...
| ログイン | `/login` | ログイン ID とパスワードで認証（**案A・2026-08-28 殿裁可**）。未ログインで保護面へ来た者はここへ誘導される |
| ホーム | `/` | ログイン後の入口。ロールに応じた導線とログアウト |
| 管理（司会者） | `/admin` | 管理の入口（`admin` ロールのみ・非 admin は 403） |
| エピソード一覧（司会者） | `/admin/episodes` | 回の一覧と新規作成（**案A P2**） |
| エピソード詳細（司会者） | `/admin/episodes/<id>` | 回の設定・問題と正解の登録・解答者の作成/招待・進行制御盤の埋め込み（**案A P2**） |
| 解答者アカウント（司会者） | `/admin/accounts` | 解答者アカウントの作成・編集（お名前・パスワード再発行・**案A P2**） |
| ご参加いただける回（解答者） | `/episodes` | 招待された回の一覧。選ぶとその回へ参加する（**案A P2**） |
| アカウント設定 | `/me` | 自分の表示名とパスワードの変更 |
| 解答者（タブレット） | `/tablet` | −10/−1/+1/+10 で 0〜100 を作り送信（ログイン必須） |

開発環境では同一 PC の複数タブ（制御盤・TV・タブレット×人数分）で 1 卓分の動作確認ができる。本番では各端末がそれぞれクラウド公開 URL へ接続する。

### 初期管理者の投入（初回のみ）

案A では**ログインしないと制御盤にも入れない**。初回は初期管理者を環境変数から投入する
（`src/accounts/seed_admin.ts`・冪等ゆえ何度起動しても 1 件のまま）。

```bash
# 資格情報は秘密保管庫から実行時にだけ env へ渡す（リポジトリにも .env にも平文を置かない）
ADMIN_LOGIN_ID='<ログインID>' ADMIN_INITIAL_PASSWORD='<パスワード>' npm run start
# 1Password CLI を使う場合（例）:
#   ADMIN_LOGIN_ID="$(op read 'op://<vault>/<item>/username')" \
#   ADMIN_INITIAL_PASSWORD="$(op read 'op://<vault>/<item>/password')" npm run start
```

- 投入結果は起動ログに `initial admin seed: created | already_exists | not_configured` として出る
  （**資格情報そのものはログに出さない**）。
- 既存アカウントのパスワードは再投入で上書きされない（`/me` で変えたパスワードが初期値へ戻らない）。
- アカウントは `DATA_DIR`（既定 `./data`）配下の `accounts.json` に、エピソード（回・招待・参加者・
  問題）は同じ置き場の `episodes.json` に永続する。`data/` は git 管理外。
- パスワードは `node:crypto` の scrypt でハッシュ化して保存する。平文は保存も表示もしない。

### 操作の最小ループ

0. 司会者が `/admin/episodes` で回を作り、詳細で **問題と正解**を登録し、**解答者を作って招待**する
   （案A P2）。解答者はログイン後 `/episodes` の一覧から**招待された回**を選んで参加する。
1. `/login` で各自がログインする（司会者は admin アカウント、解答者は contestant アカウント）。
...
live 反映は **SSE**（Server-Sent Events）で行う（`/events?surface=control_panel|tablet|tv`・ws 依存なし）。
クライアントが申告するのは「今どの面を開いているか」だけで、購読の可否と流す投影ロールはサーバが
セッションから決める（申告値で権限は上がらない）。
