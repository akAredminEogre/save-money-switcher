...
| `role: contestant` | **解答者** | `/join`→`/tablet` | 参加・数値入力・送信のみ（入力専用・受動同期先） |
...
sequenceDiagram
    ...
    alt 非 host contestant/audience/API直叩き
...
sequenceDiagram
    ...
    Note over HB,HUB: ping 15秒 / pong 猶予 30秒 で切断確定 → contestant スロット解放・上限会計と一致
    A->>S: 再接続 と resume・contestant は不透明 resume トークンを添える
    alt 無効・失効トークン
        S->>RJ: 検証失敗
