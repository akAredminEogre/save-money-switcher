...
- ~~**参加登録（論点9改）**: ホスト画面の QR をタブレットで読み、**氏名を自己入力**。**1人=1台**。端末番号固定割当は不採用。~~ → **2026-08-28 案A 改定**: ホスト画面の QR（**`/login` URL** を符号化）をタブレットで読み、事前発行アカウントの ID/パスワードで `/login` から**ログイン**して参加。端末番号固定割当は不採用。
...
| AC ID | 合格条件 | 検証レベル | Canonical VBs |
|-------|---------|-----------|---------------|
| ~~AC-05~~（**2026-08-28 案A 改定**） | ~~ホスト画面（制御盤）が参加用クラウドURLの **QR コード**を表示する（可視要素として存在する）~~ → **差替後**: ホスト画面（制御盤）が **`/login` URL** の **QR コード**を表示する（可視要素として存在する） | Browser | VB-06 |
| ~~AC-06~~（**2026-08-28 案A 改定**） | ~~タブレットが参加URLへ接続し**氏名を自己入力**して参加でき、参加が制御盤・TV(e)へ反映される（1人=1台）~~ → **差替後**: 事前発行アカウントの ID/パスワードで **`/login`** からログインし、招待されているエピソードへ参加でき、参加が制御盤・TV(e)へ反映される | Browser, API | VB-07, VB-08 |
...
- ~~`tests/e2e/helpers/join.ts` — QR 由来の参加URL取得 → タブレット参加 → 氏名自己入力フロー。~~ → **差替後（2026-08-28 案A）**: `tests/e2e/helpers/join.ts`（または `auth.ts`）— QR 由来の `/login` URL 取得 → 事前発行アカウントでのログイン参加フロー。
...
- **必須の可視ラベル/コピーをアサート**: TV d モードの6列見出し（氏名 / 解答 / 誤差 / 増減円 / ピタリ賞 / 残額）、金額の**円**表記、初期残額 10,000 円、~~QR 表示~~ → **差替後（2026-08-28 案A）**: **`/login` URL** を符号化した QR 表示。
