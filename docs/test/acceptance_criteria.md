...
| AC-07 | 同一参加を**端末番号の固定割当では行わない**（事前氏名・座席登録の UI/API が存在しない、または未使用で成立する） | Browser | VB-09 |
...
- **プライバシー / データ取扱い**: 収集する個人データは解答者の**自己入力した氏名と当日の解答・残額**に限る。タブレットは他者情報を保持・表示しない（AC-31）。氏名は当日その場参加を前提とし、恒久的な事前氏名台帳を持たない（AC-07）。
