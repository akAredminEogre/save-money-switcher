 ...
 *
 * 読出はファイル不在を「まだ何も無い」として扱い（`undefined` を返す）、例外にしない。壊れた
 * JSON も `undefined` へ収束させる（呼出側が空集合として扱えるようにし、起動を 5xx にしない）。
 * 壊れていた実体は上書きで失わせぬよう、退避名（`.corrupt-<n>`）へ改名して残す。
 */
...
/**
 * JSON ファイルを読む。ファイルが無い、または内容が JSON として解釈できない場合は `undefined`。
 * 解釈できなかった実体は退避名へ改名して保全する（黙って失わせない）。
 */
