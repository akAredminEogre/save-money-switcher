 ...
 *
 * 内部ロール識別子（host / answerer / audience）を、全サーフェス（制御盤・タブレット・
 * TV・参加受付）が共有する可視ラベル（司会者 / 解答者 / 観客）へ写す唯一の定義点である。
 * 可視コピーは必ず本写像から供給し、内部識別子（host/answerer/audience）をユーザー可視
 ...
 * （control_panel / tv_display / answerer_tablets / join_page）はこの単一定義から型・可視
 * ラベルをともに参照する。ラベル値には内部識別子（host/answerer/audience）を一切含めない。
...
/**
 * 内部ロール識別子から可視ラベルへの写像（host→司会者 / answerer→解答者 / audience→観客）。
 *
 * 全サーフェスの可視コピーはここから供給し、`host` / `answerer` / `audience` の内部識別子を
